...
- **`db:push` leaves a database the migrator cannot run against**: `drizzle-kit push` applies the schema directly and never records anything in `drizzle.__drizzle_migrations`, so a dev database built that way has the tables but an empty journal. `bun run db:migrate` then concludes nothing has been applied and replays `0000_init` into `relation "collections" already exists` (42P07) — which means the one command that rehearses a production migration is exactly the one that cannot run. Production never hits this: compose runs the migrator against an empty database, so its journal is populated from the first deploy. To repair a pushed database, first prove it matches the migrations, then record them as applied:

  ```bash
  createdb icons0_check && psql -d icons0_check -c "CREATE EXTENSION vector; CREATE EXTENSION pg_textsearch"
  DATABASE_URL=…/icons0_check bunx drizzle-kit migrate
  diff <(pg_dump --schema-only --schema=public …/icons0) \
       <(pg_dump --schema-only --schema=public …/icons0_check)   # must be empty
  # then copy icons0_check's drizzle.__drizzle_migrations rows into icons0
  ```

  Copy the rows rather than recomputing the hashes — the migrator wrote them. There is deliberately no `db:baseline` script: marking every migration applied is safe only when the schema already matches, and that diff is the part that cannot be automated away.
